...
import SignedSmallIntType from '../../src/types/SignedSmallIntType.js';
...
test('SignedSmallIntType property tests', (t) => {
  ...
  t.true(signedSmallIntType === SignedSmallIntType.create());
...
test('SignedSmallIntType decode tests', async (t) => {
  const field = new Field({ name: 'test', type: SignedSmallIntType.create() });
  const samples = [
    { input: -32768, output: -32768 },
    { input: 32767, output: 32767 },

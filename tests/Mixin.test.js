...
import SampleMixin from './Fixtures/SampleMixin';

test('Mixin tests', (t) => {
  const mixin = SampleMixin.create();

  t.true(mixin instanceof Mixin, 'mixin MUST be an instanceOf Mixin');
  t.true(mixin instanceof SampleMixin, 'mixin MUST be an instanceOf SampleMixin');
  t.true(mixin === SampleMixin.create(), 'SampleMixin.create() must return the same instance');
